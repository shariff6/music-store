...
describe(Artist) do
  ...
  describe('#name') do
    it('returns the name of the artist') do
      test_artist = Artist.new('james', nil)
      expect(test_artist.name).to(eq('james'))
    end
  end
  describe('#image') do
    it('returns the image of the artist') do
      test_artist = Artist.new('james', nil)
      expect(test_artist.image).to(eq(nil))
    end
  end
  describe('#id') do
  ...
  describe('.find') do
    it('returns an artist by their id number') do
      test_artist = Artist.new('james', nil)
      test_artist.save
      test_artist2 = Artist.new('james', nil)
      test_artist2.save
